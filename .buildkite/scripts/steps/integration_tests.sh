...
set +e
AGENT_STACK_VERSION="${STACK_VERSION}" TEST_INTEG_CLEAN_ON_EXIT=true STACK_PROVISIONER="$STACK_PROVISIONER" TEST_RUN_UNTIL_FAILURE=true TEST_PACKAGES="zip" TEST_PLATFORMS="windows" GOTEST_FLAGS='-test.run ^TestEscapingSecretsInPolicy$' SNAPSHOT=true mage $MAGE_TARGET $MAGE_SUBTARGET
TESTS_EXIT_STATUS=$?
set -e
